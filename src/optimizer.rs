...
impl OptimizationPass {
            ...
            matcher: |hir| match hir {
                HIR::Compose(f, g) => {
                    if let (HIR::Map(inner_f), HIR::Map(inner_g)) = (f.as_ref(), g.as_ref()) {
                        Some(HIR::Map(Box::new(HIR::Compose(inner_f.clone(), inner_g.clone()))))
                    } else {
                        None
                    }
                }
    ...
    fn inline_functions_recursive(&self, hir: HIR) -> HIR {
        match hir {
            HIR::App { func, args } if matches!(func.as_ref(), HIR::Var(_)) => {
                let name = if let HIR::Var(name) = func.as_ref() { name } else { unreachable!() };
            ...
            HIR::Map(f) => {
                HIR::Map(Box::new(self.inline_functions_recursive(*f)))
            }
    ...
    pub fn fuse_list_operations(&self, hir: HIR) -> HIR {
        ...
        match hir {
            ...
            HIR::Map(f) => {
                HIR::Map(Box::new(self.fuse_list_operations(*f)))
            }
            HIR::Function { name, params, body, pre, post, decreases } => {
                HIR::Function {
                    name,
    ...
    pub fn eliminate_unnecessary_clones(&self, hir: HIR) -> HIR {
        ...
        match hir {
            ...
            HIR::Map(f) => {
                HIR::Map(Box::new(self.eliminate_unnecessary_clones(*f)))
            }
            HIR::Function { name, params, body, pre, post, decreases } => {
                HIR::Function {
                    name,
    ...
    fn cse_recursive(&self, hir: HIR, cse_map: &mut HashMap<String, Symbol>, counter: &mut usize) -> HIR {
        // Hash the expression
        ...
        match hir {
            ...
            HIR::Map(f) => {
                HIR::Map(Box::new(self.cse_recursive(*f, cse_map, counter)))
            }
            HIR::Function { name, params, body, pre, post, decreases } => {
