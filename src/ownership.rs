...
impl OwnershipAnalysis {
    ...
    pub fn analyze(&mut self, hir: &HIR) -> Result<OwnershipMap> {
        ...
        for node in self.usage_graph.node_indices() {
            ...
            ownership.insert(symbol.clone(), self.infer_ownership_kind(symbol));
        }
    ...
    fn add_usage(&mut self, symbol: Symbol, kind: UsageKind) {
        if let Some(&node) = self.symbol_to_node.get(&symbol) {
            ...
            self.usage_graph
                .add_edge(node, node, UsageEdge { kind, count: 1 });
        }
    }
...
pub fn annotate_ownership(hir: &HIR, ownership_map: &OwnershipMap) -> HIR {
    match hir {
        HIR::Function {
            ...
            let annotated_params = params
                .iter()
                .map(|(sym, ty)| {
                    let ownership = ownership_map
                        .get(sym)
                        .cloned()
        ...
        HIR::Match {
            ...
            arms: arms
                ...
                .map(|(pat, expr)| {
                    (
                        pat.clone(),
                        Box::new(annotate_ownership(expr, ownership_map)),
                    )
                })
                .collect(),
        ...
        HIR::Map(f) => HIR::Map(Box::new(annotate_ownership(f, ownership_map))),
